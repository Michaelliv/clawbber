# Private Project Report — clawbber

Date: 2026-02-25

## Current State

clawbber is running as a NanoClaw-style orchestrator with:
...
- per-thread workspace isolation under `CLAWBBER_GROUPS_DIR`
- global pi environment mounted from host `CLAWBBER_PI_AGENT_DIR` to `/home/node/.pi/agent`

## Key Runtime Behavior

### Session model
- pi native session persistence is enabled (`--session <groupWorkspace>/.clawbber.session.json`)
...
### Per-session env
- Source root: `CLAWBBER_GROUPS_DIR`
...
- Host runtime session: `tmux` session `clawbber-chat` (terminated per user request)
